...
const CREATOR_CONTENT_STORAGE_KEY = "z3_minimax_creator_palette.editor_content.v1";
...
// The embedded editor follows Prompt Palette's appearance, but its library
// content/history belongs to Creator. Keeping these buckets under a Creator-
// only key prevents either node from inheriting or overwriting the other's
// custom paths, recent wildcards, pins, drafts or browsing state.
const CREATOR_EDITOR_CONTENT_KEYS = new Set([
  "pinned", "expandedCategories", "categoryOrder", "pickerView", "libraryDensity",
  "stash", "libraryWidth", "libraryTab", "stashTab",
]);
function readCreatorContent() {
  try {
    const parsed = JSON.parse(localStorage.getItem(CREATOR_CONTENT_STORAGE_KEY) || "null");
    return parsed && typeof parsed === "object" && !Array.isArray(parsed) ? parsed : {};
  } catch { return {}; }
}
function readCreatorContentPreference(section, key, fallback = null) {
  const bucket = readCreatorContent()[section];
  return bucket && Object.prototype.hasOwnProperty.call(bucket, key) ? clone(bucket[key]) : clone(fallback);
}
function writeCreatorContentPreference(section, key, value) {
  const current = readCreatorContent();
  const next = { ...current, [section]: { ...(current[section] || {}), [key]: clone(value) } };
  try { localStorage.setItem(CREATOR_CONTENT_STORAGE_KEY, JSON.stringify(next)); } catch {}
  return value;
}
...
export function readEditorPreference(key, fallback = null) {
  if (CREATOR_EDITOR_CONTENT_KEYS.has(key)) return readCreatorContentPreference("editor", key, fallback);
  return readPromptPaletteState("editor", key, fallback);
}

export function writeEditorPreference(key, value) {
  if (CREATOR_EDITOR_CONTENT_KEYS.has(key)) return writeCreatorContentPreference("editor", key, value);
  return writePromptPaletteState("editor", key, value);
}

export function readQuicknessPreference(key, fallback = null) {
  return readCreatorContentPreference("quickness", key, fallback);
}

export function writeQuicknessPreference(key, value) {
  return writeCreatorContentPreference("quickness", key, value);
}

export function readStarterPreference(key, fallback = null) {
  return readCreatorContentPreference("starter", key, fallback);
}

export function writeStarterPreference(key, value) {
  return writeCreatorContentPreference("starter", key, value);
}

export function readCombinatorialPreference(key, fallback = null) {
  return readCreatorContentPreference("combinatorial", key, fallback);
}

export function writeCombinatorialPreference(key, value) {
  return writeCreatorContentPreference("combinatorial", key, value);
}
