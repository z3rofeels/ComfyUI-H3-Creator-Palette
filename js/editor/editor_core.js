...
import { insertPromptText } from "../prompt_library.js";
...
import { createSyntaxHighlighter, renderFallbackDecorationLines } from "./syntax_highlighter.js";

function buildWildcardWidget(node, hiddenWidget, options = {}) {
  ...
  // Creator Palette embeds Prompt Palette as its editor, but owns its own
  // complete Setup / Settings surface. Do not expose the standalone Prompt
  // Palette settings drawer inside hosts that explicitly disable it.
  if (options.settings === false) root.querySelector('[data-act="settings"]')?.remove();
  ...
  function useVariationSeed(...args) { return powerToolsController?.useVariationSeed(...args); }
  ...
  const socketRailEnabled = options.ioRail !== false;
  const ioRail = socketRailEnabled
    ? setupIoRail(node, root, IO_INPUT_DEFS, IO_OUTPUT_DEFS)
    : { render() {}, close() {}, cleanup() {} };

  if (socketRailEnabled) {
    ...
    root.querySelector('[data-act="ioRailToggle"]')?.remove();
    delete node._wgRefreshIoToggles;
  }
  ...
  function highlightText(text, parsedOverride = null) {
    ...
    let result = profiler.measure("editor.highlight", () => renderPromptSyntax(parsed, {
      isKnown,
      categoryOf,
    ...
    try {
      if (typeof node?._ppDecoratePrompt === "function") result = node._ppDecoratePrompt(text, result) || result;
    } catch (error) { console.error("Prompt Palette: custom prompt decoration failed", error); }
  ...
  function renderVisualsFromParsed(parsedOverride = null, sourceText = textarea.value) {
    ...
    try {
      ...
      profiler.measure("editor.dom", () => {
        if (!syntaxHighlighter.render(result.decorations, sourceText)) {
          syncHighlightDom(renderFallbackDecorationLines(sourceText, result.decorations), sourceText);
        }
      });
    } catch (e) {
      ...
      // Never reveal the compatibility mirror behind a visible single-surface
      // editor. If decoration parsing fails, keep editing plain text and retry
      // semantic highlighting on the next render without duplicating glyphs.
      highlight.style.display = syntaxHighlighter.native ? "none" : "";
      if (!syntaxHighlighter.native) highlight.replaceChildren(document.createTextNode(sourceText + (sourceText.endsWith("\n") ? "\n" : "")));
    }
  ...
  async function refreshResolvedView() {
    ...
    try {
      ...
      try {
        if (typeof node?._ppResolveSource === "function") resolveSource = String(node._ppResolveSource(source) ?? source);
      } catch (error) { console.error("Prompt Palette: custom preview source expansion failed", error); }
      const result = await profiler.measureAsync("preview.resolve", () => API.resolve(resolveSource, seed, mode, { signal: requestController.signal }));
      performanceRuntime.endRequest("preview", requestController);
      if (!stillCurrent()) return;
  ...
  const commandTarget = registerPromptPaletteCommandTarget(node, {
    ...
    openVariations,
    openLibraryManager,
    insertWildcard: (path) => libraryController?.insertWildcard?.(path),
    toggleFavorite: (path) => libraryController?.togglePinned?.(path),
    insertText: (value) => insertPromptText(textarea, String(value ?? "")),
    useVariationSeed: (seed, options = {}) => useVariationSeed(seed, options),
    getWorkspaceContext: () => ({
      nodeId: node.id == null ? null : String(node.id),
      title: String(node.title || node.comfyClass || "Prompt Palette"),
      text: String(textarea.value || ""),
      seed: Number(seedWidget?.value ?? 0),
      mode: String(modeWidget?.value || "entire text as one"),
      promptDoctorEnabled: !!powerTools.promptDoctor,
      variationLabEnabled: !!powerTools.variationLab,
      starterPacksEnabled: theme.starterPacksEnabled !== false,
    }),
  });
