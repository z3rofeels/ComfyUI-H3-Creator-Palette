...
import {
  loadPinned, savePinned, saveExpandedCats, saveCatOrder, savePickerView,
} from "./preferences.js";
import {
...
export function createLibraryController(ctx) {
  ...
  function syncPinnedFromPreferences() {
    const latest = loadPinned();
    pinned.clear();
    for (const path of latest) pinned.add(path);
  }

  function togglePinned(path) {
    const key = String(path || "").trim();
    if (!key) return false;
    syncPinnedFromPreferences();
  ...
  async function getAcMatches(query) {
    ...
    const pushLibrary = (path, group, kind = "library", favorite = false, allowFuzzy = false) => {
      if (!path || !state.knownSet.has(path) || used.has(`library:${path}`) || (!allowFuzzy && !contains(path))) return;
  ...
  function openLibraryContextMenu(event, path) {
    ...
    openCtxMenu(event.clientX, event.clientY, [
      ...
      { label: nowPinned ? "Unpin" : "Pin", onSelect: () => togglePinned(path) },
