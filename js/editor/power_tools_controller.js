...
export function createPowerToolsController(ctx) {
  ...
  function normalizedVariationSeed() {
    const candidates = [seedInput?.value, seedWidget?.value];
    for (const raw of candidates) {
      if (typeof raw === "bigint") {
        const max = BigInt(Number.MAX_SAFE_INTEGER);
        return Number(raw < 0n ? 0n : raw > max ? max : raw);
      }
    ...
    // ComfyUI can expose control-after-generate state through a linked widget
    // while the visible seed is being randomized/incremented. Variation Lab is
    // preview-only, so use a deterministic safe seed instead of surfacing a
    // backend validation error or mutating the real render seed.
    return 0;
  }
  ...
  function renderVariations(items) {
    ...
    items.forEach((item, index) => {
      ...
      const variationStep = Number.isFinite(Number(item?.variation_index)) ? ` · var ${Number(item.variation_index) + 1}` : "";
      const rightMeta = item?.meta ? String(item.meta) : `${Number.isFinite(tokens) ? `${tokens} tokens · ` : ""}${wildcardCount} wildcard${wildcardCount === 1 ? "" : "s"}`;
      card.innerHTML = `<div class="wg-variation-meta"><strong>#${index + 1} · seed ${escapeHtml(String(item.seed ?? ""))}${escapeHtml(variationStep)}</strong><span>${escapeHtml(rightMeta)}</span></div><div class="wg-variation-text"></div><div class="wg-power-actions"><button type="button" data-use>Use seed</button><button type="button" data-preview>Use + preview</button><button type="button" data-copy>Copy resolved</button></div>`;
      card.querySelector(".wg-variation-text").textContent = String(item.resolved || "");
      const useItem=(preview=false)=>{
        const requestedVariation=Number(item?.variation_index);
        if(Number.isFinite(requestedVariation)){
          const value=Math.max(0,Math.trunc(requestedVariation)),body=node?._z3CreatorBody;
          if(typeof body?.setVariationIndex==="function")body.setVariationIndex(value,false);
          else{const variationWidget=node?.widgets?.find?.((widget)=>widget?.name==="variation_index");if(variationWidget){variationWidget.value=value;variationWidget.callback?.(value,node.graph?.canvas,node);}}
          node.graph?.setDirtyCanvas?.(true,true);
        }
        useVariationSeed(item.seed,{preview});
        if(!preview)setVariationStatus(Number.isFinite(Number(item?.variation_index))?`Variation ${Number(item.variation_index)+1} · seed ${item.seed} is now active.`:`Seed ${item.seed} is now active.`);
      };
      card.querySelector("[data-use]").textContent=Number.isFinite(Number(item?.variation_index))?"Use variation":"Use seed";
      card.querySelector("[data-preview]").textContent=Number.isFinite(Number(item?.variation_index))?"Use + preview":"Use + preview";
      card.querySelector("[data-use]").addEventListener("click",()=>useItem(false));
      card.querySelector("[data-preview]").addEventListener("click",()=>useItem(true));
  ...
  async function resolveVariationItems(source, seed, mode, count, requestController) {
    if (typeof node?._ppResolveVariations === "function") {
      return await node._ppResolveVariations({ source, seed, mode, count, signal: requestController.signal, node, textarea, ctx });
    }
    return await API.resolveVariations(source, seed, mode, count, { signal: requestController.signal });
  }
  ...
  async function refreshVariations() {
    ...
    const seed = normalizedVariationSeed();
    ...
    const stillCurrent = () => requestId === variationRequestId
      ...
      && normalizedVariationSeed() === seed
      && (modeWidget?.value || "entire text as one") === mode;
    setVariationStatus(`Resolving ${count} versions from seed ${seed}…`);
    variationList.innerHTML = "";
    const requestController = performanceRuntime?.beginRequest("variations") || new AbortController();
    try {
      const resolver = () => resolveVariationItems(source, seed, mode, count, requestController);
      const items = await profiler?.measureAsync("variations.resolve", resolver) ?? await resolver();
      ...
      setVariationStatus(hasUnseeded
        ? `${items.length} seed-stepped version${items.length === 1 ? "" : "s"} from ${seed}. Unseeded * / ~ syntax remains intentionally random. Nothing was queued.`
        : `${items.length} deterministic version${items.length === 1 ? "" : "s"} from seed ${seed}. No generation was queued.`);
    } catch (error) {
      performanceRuntime?.endRequest("variations", requestController);
