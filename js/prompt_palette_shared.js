 ...
 * canvas nodes.
 */
export function applyPromptPaletteNodeChrome(node, colors = {}, { accentBorder = false } = {}) {
  if (!node) return;
  const graph = node.graph;
  const changes = [
    ["color", colors.accent],
    ["bgcolor", colors.bg],
    ["boxcolor", accentBorder ? colors.accent : (colors["border-strong"] || colors.border || colors.bg)],
  ];
