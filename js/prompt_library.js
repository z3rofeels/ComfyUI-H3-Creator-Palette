...
export function loadStarterPackCatalog() {
  if (!catalogPromise) {
    catalogPromise = H3PackAPI.load().then((pack) => pack.catalog).catch((error) => { catalogPromise = null; throw error; });
  }
  return catalogPromise;
}

if (typeof window !== "undefined") window.addEventListener?.("z3-h3-pack-changed", () => { catalogPromise = null; });
...
export function insertPromptText(textarea, value) {
